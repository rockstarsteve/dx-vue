...
const HexString2Bytes = function HexString2Bytes(str) {
  var pos = 0;
  var len = str.length;
  if (len % 2 != 0) {
    return null;
...
//字节数组转十六进制字符串，对负值填坑
const Bytes2HexString = function Bytes2HexString(arrBytes) {
  var str = "";
...
/**
 * ECB加密模式
 * @example encrypt_cbc("00000000000000000000000000000000", "00000000000000000000000000000000") => "9F1F7BFF6F5511384D9430531E538FD3"
 * @param {any} plaintext 要加密的数据(hex)
 * @param {String} key 加密秘钥
 * @param {String} iv 
 * @param {String} mode base64 | "text"
 * @returns {String} 加密后的字符串
 */
 const encryptHex_ecb = (plaintext, key, mode = "base64",iv="") => {
  // if (!check("iv", iv)) { return; }
  console.log(plaintext, key)
  let encryptRoundKeys = EncryptRoundKeys(HexString2Bytes(key));
  let plainByteArray = HexString2Bytes(plaintext);
  // let padded = padding(plainByteArray);
  let padded = plainByteArray;
  let blockTimes = padded.length / UINT8_BLOCK;
  let outArray = [];
  // CBC mode
  // init chain with iv (transform to uint32 block)
  ...
  return Bytes2HexString(outArray)
} 
 ...
 * @returns {String} 解密后的字符串
 */
const decrypt_ecb = (ciphertext, key, mode = "base64", iv) => {
  if (!check("iv", iv)) { return; }
  // get cipher byte array
  let decryptRoundKeys = EncryptRoundKeys(stringToArray(key)).reverse();
  let cipherByteArray = null;
